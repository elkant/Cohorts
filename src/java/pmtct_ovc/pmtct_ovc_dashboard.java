 ...
 * @author EKaunda
 */
public class pmtct_ovc_dashboard extends HttpServlet {
...
String allpath = getServletContext().getRealPath("/pmtct_ovc_dashboard.xlsx");
...
//                    
        ...
        String qry = " SELECT * FROM internal_system.vw_motherbaby_clinicalovc_dashboard where `Linelist Month` between '"+sd+"' and '"+ed+"';";
...
((XSSFSheet)sheet).getCTWorksheet().getDimension().setRef("A1:AL" + (shet.getLastRowNum() + 1));

CTTable ctTable = ((XSSFSheet)sheet).getTables().get(0).getCTTable();

ctTable.setRef("A1:AL" + (sheet.getLastRowNum() + 1)); // adjust reference as needed
...
//            wb.setWorkbookPassword(password, HashAlgorithm.sha512);
        ...
        System.out.println("" + "PMTCT_OVC_Reports_Gen_" + createdOn.trim() + ".xlsx");
        ...
        response.setHeader("Content-Disposition", "attachment; filename=" + "PMTCT_OVC_Dashboard_btwn_"+startdate+"_to_"+enddate+"_gen_" + createdOn.trim() + ".xlsx");
