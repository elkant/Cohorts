 ...
 * @author EKaunda
 */
public class qoctracker extends HttpServlet {
...
String allpath = getServletContext().getRealPath("/qoc_tracker.xlsx");
...
String filepath="QOC_TRACKER"+dat2+".xlsx";
...
String sourcepath = getServletContext().getRealPath("/qoc_tracker.xlsx");
...
//        //county
        ...
        String orgunits="  ( `aphiaplus_moi`.`vw_weekly_surge_qoc`.`Reporting Date` between  '"+startdate+"' and '"+enddate+"' )  ";
      ...
      subcountyar=request.getParameterValues("subcounty"); 
       ...
       // cell.setCellValue("Surge Tracker for Period "+startdate+"  to "+enddate+"");
        ...
        String qry = "SELECT * FROM `aphiaplus_moi`.`vw_weekly_surge_qoc` where "+orgunits+" ;";
...
ctTable.setRef("A1:I" + (shet.getLastRowNum() + 1)); // adjust reference as needed
        ...
        System.out.println("" + "QOC_Tracker_" + createdOn.trim() + ".xlsx");
        ...
        response.setHeader("Content-Disposition", "attachment; filename=" + "QOC_Tracker_"+startdate+"_to_"+enddate+"_gen_" + createdOn.trim() + ".xlsx");
