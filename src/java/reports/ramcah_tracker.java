 ...
 * @author EKaunda
 */
public class ramcah_tracker extends HttpServlet {
...
String allpath = getServletContext().getRealPath("/ramcah_tracker.xlsx");
...
String filepath="RMCAH_TRACKER"+dat2+".xlsx";
...
String sourcepath = getServletContext().getRealPath("/ramcah_tracker.xlsx");
...
//        //county
        ...
        String orgunits="  ( `aphiaplus_moi`.`vw_weekly_surge_rmcah`.`Reporting Date` between  '"+startdate+"' and '"+enddate+"' )  ";
      ...
      subcountyar=request.getParameterValues("subcounty"); 
       ...
       // cell.setCellValue("Surge Tracker for Period "+startdate+"  to "+enddate+"");
        ...
        String qry = "SELECT * FROM `aphiaplus_moi`.`vw_weekly_surge_rmcah` where "+orgunits+" ;";
...
ctTable.setRef("A1:I" + (shet.getLastRowNum() + 1)); // adjust reference as needed
        ...
        System.out.println("" + "Ramcah_Tracker_" + createdOn.trim() + ".xlsx");
        ...
        response.setHeader("Content-Disposition", "attachment; filename=" + "Ramcah_Tracker_"+startdate+"_to_"+enddate+"_gen_" + createdOn.trim() + ".xlsx");
