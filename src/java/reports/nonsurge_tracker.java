 ...
 * @author EKaunda
 */
public class nonsurge_tracker extends HttpServlet {
...
String allpath = getServletContext().getRealPath("/surgetracker.xlsx");
...
String filepath="Surge_Tracker"+dat2+".xlsx";
...
String sourcepath = getServletContext().getRealPath("/non_surge_sites_tracker.xlsx");
...
//        //county
        ...
        //SELECT * FROM pews.hfr_tracker where `Date` between '2020-01-20' and '2020-02-10';
        
        String orgunits="  ( hfr_tracker.`Date` between  '"+startdate+"' and '"+enddate+"' )  ";
...
//        cell.setCellValue("Non-Surge Tracker for Period "+startdate+"  to "+enddate+"");
...
//        shet.addMergedRegion(new CellRangeAddress(1, 1, 0,10));
        ...
        String qry = "SELECT * FROM pews.hfr_tracker where "+orgunits+" ;";
...
((XSSFSheet)shet).getCTWorksheet().getDimension().setRef("A1:I" + (shet.getLastRowNum() + 1));

CTTable ctTable = ((XSSFSheet)shet).getTables().get(0).getCTTable();

ctTable.setRef("A1:I" + (shet.getLastRowNum() + 1)); // adjust reference as needed
        ...
        System.out.println("" + "tracker_reports_Gen_" + createdOn.trim() + ".xls");
        ...
        response.setHeader("Content-Disposition", "attachment; filename=" + "Non surge Tracker_"+startdate+"_to_"+enddate+"_gen_" + createdOn.trim() + ".xlsx");
